...
/*
  Command_Packet represents the 12 char command that we send to the finger print scanner
*/
class Command_Packet
{
  public:
    ...
    char Parameter[4];                // Parameter 4 chars, changes meaning depending on command              
    char* GetPacketBytes();             // returns the chars to be transmitted
    void ParameterFromInt(int i);

    Command_Packet();

  private: 
    static const char COMMAND_START_CODE_1 = 0x55;  // Static char to mark the beginning of a command packet  - never changes
    static const char COMMAND_START_CODE_2 = 0xAA;  // Static char to mark the beginning of a command packet  - never changes
    static const char COMMAND_DEVICE_ID_1 = 0x01; // Device ID Byte 1 (lesser char)             - theoretically never changes
    static const char COMMAND_DEVICE_ID_2 = 0x00; // Device ID Byte 2 (greater char)              - theoretically never changes
    char command[2];                // Command 2 chars

    word _CalculateChecksum();            // Checksum is calculated using char addition
    char GetHighByte(word w);           
    char GetLowByte(word w);
};
#pragma endregion
...
class Response_Packet
{
  public:
    class ErrorCodes
    {
      public:
        ...
        static Errors_Enum ParseFromBytes(char high, char low);
    };
    Response_Packet(char* buffer, bool UseSerialDebug);
    ErrorCodes::Errors_Enum Error;
    char RawBytes[12];
    char ParameterBytes[4];
    char ResponseBytes[2];
    bool ACK;
    static const char COMMAND_START_CODE_1 = 0x55;  // Static char to mark the beginning of a command packet  - never changes
    static const char COMMAND_START_CODE_2 = 0xAA;  // Static char to mark the beginning of a command packet  - never changes
    static const char COMMAND_DEVICE_ID_1 = 0x01; // Device ID Byte 1 (lesser char)             - theoretically never changes
  ...
  private: 
    bool CheckParsing(char b, char propervalue, char alternatevalue, char* varname, bool UseSerialDebug);
    word CalculateChecksum(char* buffer, int length);
    char GetHighByte(word w);           
    char GetLowByte(word w);
};
#pragma endregion
...
// Data Mule packet for receiving large data(in 128 char pieces) from the FPS
...
class FPS_GT511C3
{
 ...
 public:
  ...
  // Not implemented due to memory restrictions on the arduino
  // may revisit this if I find a need for it
  //int SetTemplate(char* tmplt, int id, bool duplicateCheck);
  ...
  void serialPrintHex(char data);
  void SendToSerial(char data[], int length);
...
/*
 * Class to replace calls to Arduino's Serial class for outputting to stdout.
 */
class Serial {
  public:
