/* 
  ...
  Modified for Beaglebone use by Drew Gottlieb.
*/
...
char* Command_Packet::GetPacketBytes()
{
  char* packetchars= new char[12];
...
// Returns the high char from a uint16_t
char Command_Packet::GetHighByte(uint16_t w)
{
  return (char)(w>>8)&0x00FF;
}

// Returns the low char from a uint16_t
char Command_Packet::GetLowByte(uint16_t w)
{
...
Response_Packet::Response_Packet(char* buffer, bool UseSerialDebug)
{
  ...
  char checksum_low = GetLowByte(checksum);
  char checksum_high = GetHighByte(checksum);
...
Response_Packet::ErrorCodes::Errors_Enum Response_Packet::ErrorCodes::ParseFromBytes(char high, char low)
{
...
uint16_t Response_Packet::CalculateChecksum(char* buffer, int length)
{
  uint16_t checksum = 0;
...
// Returns the high char from a uint16_t
char Response_Packet::GetHighByte(uint16_t w)
{
  return (char)(w>>8)&0x00FF;
}

// Returns the low char from a uint16_t
char Response_Packet::GetLowByte(uint16_t w)
{
  return (char)w&0x00FF;
}

// checks to see if the char is the proper value, and logs it to the serial channel if not
bool Response_Packet::CheckParsing(char b, char propervalue, char alternatevalue, char* varname, bool UseSerialDebug)
{
  bool retval = (b != propervalue) && (b != alternatevalue);
  if ((UseSerialDebug) && (retval))
  {
    std::cout << "Response_Packet parsing error ";
    std::cout << varname;
    std::cout << " ";
    std::cout << std::hex << propervalue << std::dec;
    std::cout << " || ";
    std::cout << std::hex << alternatevalue << std::dec;
    std::cout << " != ";
...
//  203 - Device error
  // Not implemented due to memory restrictions on the arduino
  // may revisit this if I find a need for it
//int FPS_GT511C3::SetTemplate(char* tmplt, int id, bool duplicateCheck)
//{
  // Not implemented due to memory restrictions on the arduino
  // may revisit this if I find a need for it
  //return -1;
...
void FPS_GT511C3::SendCommand(char cmd[], int length)
{
...
Response_Packet* FPS_GT511C3::GetResponse()
{
  ...
  while (done == false)
  {
    firstchar = (char)_serial.read();
    if (firstchar == Response_Packet::COMMAND_START_CODE_1)
    {
      done = true;
    }
  }
  ...
  for (int i=1; i < 12; i++)
  {
    while (_serial.available() == false) usleep(1000 * 10);
    resp[i]= (char) _serial.read();
  }
...
void FPS_GT511C3::SendToSerial(char data[], int length)
{
  boolean first=true;
  Serial.print("\"");
  for(int i=0; i<length; i++)
...
// sends a char to the serial debugger in the hex format we want EX "0F"
void FPS_GT511C3::serialPrintHex(char data)
{
  char tmp[16];
  sprintf(tmp, "%.2X",data); 
  Serial.print(tmp); 
